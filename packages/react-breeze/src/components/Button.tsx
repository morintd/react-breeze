...
function Button(props: Props) {
  ...
  const classnames = cn(className, 'py-1.5 px-3 rounded border-2', {
    [`bg-${color}-500 text-${color}-inverted border-${color}-500`]: !outline,
    [`hover:bg-${color}-600 active:bg-${color}-700`]: !loading && !disabled && !outline,
    [`bg-${color}-inverted text-${color}-500 border-${color}-500`]: outline,
  ...
  return (
    <button {...others} onClick={handleClick} className={classnames}>
      <div className="relative">
        {loading && <Spinner color={color} inverted={!outline} className="absolute top-0 left-0 right-0 mx-auto" />}
